...
def create_network(mean_deg_single, mean_deg_triangle, num_nodes):
    # Ensure the number of single edges is even
    while True:
        single_edges = np.random.poisson(mean_deg_single, num_nodes)
        if np.sum(single_edges) % 2 == 0:
            break
    # Ensure the number of triangle edges is a multiple of 3
    while True:
        triangle_edges = np.random.poisson(mean_deg_triangle, num_nodes)
...
def run_experiment(i, mean_deg_single, mean_deg_triangle, num_nodes, transmission_list, mutation_prob):
    network = create_network(mean_deg_single, mean_deg_triangle, num_nodes)  # network = input a real social network
...
if __name__ == '__main__':
    ...
    for mean_degree in mean_degree_list:
        meanDegSingle = meanDegTriangle = mean_degree
        a = time.time()
        ttlEpidemicsSize = 0
        numEpidemics = 0
        Epidemics = []
        ...
        Parallel(n_jobs=num_cores)(
            delayed(run_experiment)(i, meanDegSingle, meanDegTriangle, num_nodes, T_list, mutation_probability)
            for i in range(numExp))

        for ii in range(numExp):
            # print('exp', ii)
            # print('intermediate results')
            resultsFrac = ('meanDegS: {0} meanDegT: {1} Size: {2} infSt1: {3} infSt2: {4}\n'
                           .format(meanDegSingle, meanDegTriangle, fraction_dict[ii],
        ...
        print('printing for', mean_degree)
        results = 'numExp: {0} Threshold: {1} n: {2} meanDeg: {3} Prob: {4} AvgValidSize: {5} StdValidSize: {6} \
        infSt1: {7} infSt2: {8} AvgSize: {9} T: {10} Mu: {11} Time: {12} numEpidemics: {13} \n' \
            .format(numExp, thrVal, num_nodes, mean_degree, numEpidemics * 1.0 / numExp,
